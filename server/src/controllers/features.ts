...
const getLeaderboardInfo = TryCatch(async (req: Request, res: Response) => {
  ...
  if (
    ...
    return sendResponse(400, false, "Invalid pagination parameters", res);
  }
  if (pageNumber > totalPages) {
    return sendResponse(404, false, "Page not found", res);
  }
...
const getDailyStreakData = TryCatch(async (req: Request, res: Response) => {
  const { userId } = req.body;
  if (!userId) {
    return sendResponse(400, false, "User ID is required", res);
  }
  ...
  sendResponse(200, true, "Daily streak data fetched successfully", res, data);
});

const dailyUpdate = TryCatch(async (req: Request, res: Response) => {
