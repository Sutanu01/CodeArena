...
async function runSubmission(
  ...
  try {
    ...
    console.log("Judge0 response status:", postResp.status);
    
    if (!postResp.ok) {
      const errorText = await postResp.text();
      console.error("Judge0 error response:", errorText);
      throw new Error(`Failed to submit code: ${postResp.status} ${postResp.statusText} - ${errorText}`);
    }
    ...
    while (attempts < maxAttempts) {
      ...
      if (!getResp.ok) {
        ...
        throw new Error(`Failed to get submission result: ${getResp.status} ${getResp.statusText}`);
      }
...
export const SubmitCode = TryCatch(async (req: Request, res: Response) => {
  interface Body {
    questionId: string;
    source_code: string;
    language: LanguageId;
  ...
  const { questionId, source_code, language } = req.body as Body;

  if (!questionId || !source_code || !language) {
    return sendResponse(400, false, "Missing required fields: questionId, source_code, and language", res, null);
  }
  ...
  console.log("Found test case with", questionTestCase.stdin.length, "individual tests");

  try {
    const results: TestResult[] = [];

    for (let i = 0; i < 2; i++) { //um bro thora garibi hia isliye 2 pe limit kiya hu , agar mera key khatam hojaye tera laga dena if you wanna test
    ...
    return sendResponse(200, true, "Submission completed", res, {
      questionId,
      summary: {
        total: totalTests,
        passed: passedTests,
    ...
    return sendResponse(500, false, "Failed to process submission", res, null);
  }
});